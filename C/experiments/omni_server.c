...
struct connection_state {
  enum { SEND_HEADER,
         ...
         CREATE_PNG_2,
        FINISHED,
      SEND_DATA_2 } state;  // What stage of the connection we're in
...
static ssize_t content_reader(void *cls, uint64_t pos, char *buf, size_t max) {
  ...
  if (conn->state == SEND_HEADER) {
    ...
    if (conn->offset >= header_len) {
      ...
      conn->frame_info = freenect_find_depth_mode(FREENECT_RESOLUTION_MEDIUM, FREENECT_DEPTH_10BIT);
      conn->is_depth = 1;
      conn->output_bit_depth = 16;
      conn->PNG_COLOR_TYPE = PNG_COLOR_TYPE_GRAY;
  ...
  if (conn->state == CREATE_PNG_2) {
    ...
    conn->state = SEND_DATA_2;
    conn->offset = 0;
    return 0;
  }
  if (conn->state == SEND_DATA_2) {
    size_t remaining = conn->mem->size - conn->offset;
    printf("REMAINING DATA: %d\n", remaining);
    if (remaining == 0) {
      printf("SENT DATA\n");
