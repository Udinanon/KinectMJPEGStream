...
def on_mouse_move(event):
    if event.inaxes:
        ...
        print(
              f"{depth_matrix[int(event.ydata), int(event.xdata)]}")
        global HOVER_VALUE
        HOVER_VALUE = depth_matrix[int(event.ydata), int(event.xdata)]
        #axs_depth.text(x=0, y=0, s=f"{depth_matrix[int(event.ydata), int(event.xdata)]}", verticalalignment='bottom',
...
fig_depth.colorbar(plt.cm.ScalarMappable(norm=None, cmap=cmap),
             ax=axs_depth[0], label="Normalized Thrust [a.u.]")
#fig_rgb, axs_rgb = plt.subplots(1, 1)
i=1
running = True
...
while running:    
    ...
    edge_detected = conv2d(depth_matrix, edge_matrix)
    ...
    # axs_depth[1].imshow(edge_detected)
